...
def xiaoqu_spider(db_xq,url_page=u"http://bj.lianjia.com/xiaoqu/pg1rs%E6%98%8C%E5%B9%B3/"):
    ...
    try:
        ...
        plain_text = source_code.decode('utf-8')  # 转换为Unicode字符串
    ...
    for xq in xiaoqu_list:
        ...
        # 找到class为positionInfo的div元素
        position_info = xq.find('div', {'class': 'positionInfo'})
        district = position_info.find('a', {'class': 'district'}).text
        bizcircle = position_info.find('a', {'class': 'bizcircle'}).text
        # 提取"塔板结合"位置的文本
        layout_text = position_info.text.split('/')[-2].strip()
        # 提取"1980-1999年"位置的文本
        build_year_text = position_info.text.split('/')[-1].split('年')[0].strip()
        ...
        info_dict.update({u'小区户型': layout_text})
        
        info_dict.update({u'建造时间': build_year_text})
        # print(info_dict)
...
def do_xiaoqu_spider(db_xq,region=u"昌平"):
                ...
                if content_type_header:
                    ...
                    for part in content_type_parts:
                        ...
                        if part.startswith('charset='):
                            encoding = part[8:].strip() or encoding
                            break

        
        plain_text = source_code.decode(encoding)  
    ...
    exec(d)
    # 将从JavaScript获取的字符串转换为Python字典
    json_data = json.loads(d[2:])  # 去掉 "d=" 前缀
    total_pages = json_data['totalPage']
    # total_pages=d['totalPage']
    
    threads=[]
    for i in range(total_pages):
        url_page = u"http://bj.lianjia.com/xiaoqu/pg{}/rs/{}/".format(i + 1, urllib.parse.quote(region))
...
def chengjiao_spider(db_cj,url_page=u"http://bj.lianjia.com/chengjiao/pg1rs%E5%86%A0%E5%BA%AD%E5%9B%AD"):
    ...
    for cj in cj_list:
        ...
        content=(cj.find('div',{'class':'con'}).renderContents().strip()).decode('utf-8')  # 转换为Unicode字符串
...
def xiaoqu_chengjiao_spider(db_cj,xq_name=u"冠庭园"):
    ...
    爬取小区成交记录
    """
    url=u"http://bj.lianjia.com/chengjiao/rs/"+urllib.request.quote(xq_name)+"/"
    try:
        req = urllib.request.Request(url,headers=hds[random.randint(0,len(hds)-1)])
        source_code = urllib.request.urlopen(req,timeout=10).read()
        plain_text=source_code.decode('utf-8')  # 转换为Unicode字符串   
